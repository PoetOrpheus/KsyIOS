...
struct Product: Identifiable, Codable, Equatable {
    ...
    let images: [String] // Список URL изображений продукта
    let imagesRes: [String] // Список имен изображений для карусели (для тестовых данных)
    ...
    func calculateDiscountPercent() -> Int? {
        guard let old = oldPrice, old > price && old > 0 else {
            return discount
        }
        return Int((Double(old - price) / Double(old)) * 100)
    }
    ...
    var accentColor: Color {
        Color(hex: accentColorHex) ?? .black
    }
}
...
struct ProductVariant: Identifiable, Codable, Equatable {
    ...
    let imagesRes: [String] // Список имен изображений варианта для карусели (для тестовых данных)
    ...
    func getFirstImageRes() -> String? {
        return imagesRes.first
    }
    ...
    func hasImages() -> Bool {
        return !imagesRes.isEmpty || !imagesUrl.isEmpty
    }
}

// MARK: - Color Extension для работы с hex
extension Color {
    init?(hex: String) {
